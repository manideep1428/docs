...
Cline is a powerful VSCode extension that combines AI assistance with terminal integration, autonomous task execution, and advanced context awareness. Configure it to use Emby for access to multiple AI models.
...
<Tabs>
  <Tab title="OpenAI Format (GPT)">
    ...
    <Steps>
      ...
      <Step title="Select OpenAI Provider">
        ...
        * **API Key**: `sk-emby-your-api-key-here`
      </Step>
    ...
    **Configuration in settings.json:**
    ...
    ```json  theme={null}
    {
      ...
      "cline.openai": {
        "apiKey": "sk-emby-your-api-key-here",
    ...
    <Info>
      **Note:** For OpenAI format, baseURL should include `/v1`: `https://dev/emby.ai/v1`
    </Info>
  ...
  <Tab title="Environment Variables">
    ...
    **Step 1**: Set environment variables
    ...
    # For GPT models
    export OPENAI_BASE_URL="https://dev/emby.ai/v1"
    ```
    ...
    **Step 3**: Reference in settings.json
    ...
    ```json  theme={null}
    {
      ...
      "cline.defaultModel": "claude-4.5-sonnet"
    }
    ```
...
<Steps>
  ...
  <Step title="Get Emby API Key">
  1. Visit [Emby Dashboard](https://dev.emby.ai/) 2. Navigate to **API
  Keys** section 3. Click **Create New Key** 4. Copy the key (starts with
  `sk-emby-`)
</Step>
  ...
  <Step title="Configure Cline">
    ...
    **For Claude Models:**
    ...
    ```json  theme={null}
    {
      "cline.apiProvider": "anthropic",
      "cline.anthropic": {
        "apiKey": "sk-emby-your-actual-key",
        "baseURL": "https://dev.emby.ai"
      },
      "cline.defaultModel": "claude-4.5-sonnet"
    }
    ```
  ...
  1. Click Cline icon in left sidebar 2. Or: `Ctrl+Shift+P` → `Cline: Open` 3.
  ...
  <Step title="Test Configuration">
    ...
    **Expected Response:**
    ...
    ```
    I'm using claude-4.5-sonnet via Emby.
...
<Steps>
  <Step title="Configure for Autonomous Mode">
    ```json  theme={null}
    {
      ...
      "cline.defaultModel": "claude-opus-4.5-20250805",
...
Switch from Copilot Chat to Cline with Emby:
...
<Steps>
  ...
  <Step title="Install Cline">VSCode Extensions → Search "Cline" → Install</Step>

  <Step title="Configure with Emby">
    ```json  theme={null}
    {
      "cline.apiProvider": "anthropic",
      "cline.anthropic": {
  ...
  * `/explain` - Explain selected code * `/test` - Generate tests * `/fix` -
  Debug and fix issues * `/refactor` - Refactor code * `/task` - Execute
  multi-step task
</Step>
...
| Feature                | Copilot Chat                                 | Cline + Emby                             |
| ---------------------- | -------------------------------------------- | ---------------------------------------- |
...
→ Use "balanced" profile (claude-4.5-sonnet)
→ Good quality, reasonable speed

Afternoon: Complex refactoring
→ Use "powerful" profile (claude-opus-4.5-20250805)
...
```json theme={null}
{
  ...
  "cline.anthropic": {
    "apiKey": "${env:EMBY_API_KEY}",
    "baseURL": "https://dev.emby.ai" // No /v1 for Anthropic
  },

  // OpenAI (GPT) Configuration
  "cline.openai": {
    "apiKey": "${env:EMBY_API_KEY}",
    "baseURL": "https://dev/emby.ai/v1" // Include /v1 for OpenAI
  },
...
| -------------------- | -------------------------- | ------------ | ------------------ |
| **Code Review**      | `claude-4.5-sonnet`        | Anthropic    | Excellent analysis |
| **Refactoring**      | `claude-opus-4.5-20250805` | Anthropic    | Best reasoning     |
...
| **Autonomous Tasks** | `claude-opus-4.5-20250805` | Anthropic    | Complex reasoning  |
...
4. Should respond with model name and Emby confirmation
    ...
    **Solutions:**
    ...
    1. **Check API provider configuration:**
       ...
       // For Claude models
       {
         "cline.apiProvider": "anthropic",
         "cline.anthropic": {
           "baseURL": "https://dev.emby.ai"  // No /v1
         }
       }
       ...
       // For GPT models
       {
         "cline.apiProvider": "openai",
         "cline.openai": {
           "baseURL": "https://dev/emby.ai/v1"  // Include /v1
         }
       }
       ```
    ...
    2. **Verify model is available:**
       ...
       curl -H "Authorization: Bearer $EMBY_API_KEY" \
            https://dev/emby.ai/v1/models | jq '.data[].id'
       ```

    3. **Check model name is correct:**
       * `claude-4.5-sonnet` <Icon icon="check" />
       * `claude-3.5-sonnet` <Icon icon="xmark" /> (old version)
  ...
  <Accordion title="API key not recognized">
    ...
    **Solutions:**
    ...
    3. **Set key directly (testing):**
       ...
       {
         "cline.anthropic": {
           "apiKey": "sk-emby-your-actual-key"  // Temporarily for testing
         }
       }
       ```
    ...
    5. **Check key is active:**
       * Login to [Dashboard](https://dev.emby.ai/)
...
<CardGroup cols={2}>
  <Card title="Use Environment Variables" icon="key">
    Keep API keys in env vars: `${env:EMBY_API_KEY}`
  </Card>
...
<CardGroup cols={3}>
  <Card title="Kilo Code Setup" icon="code" href="/integrations/kilo-code">
    Configure Kilo Code for code completion
  </Card>

  <Card title="Roo Code Setup" icon="robot" href="/integrations/roo-code">
  Configure Roo Code standalone app
  </Card>

  <Card title="Models Catalog" icon="layer-group" href="/core-features/models">
  Browse all available models
  </Card>

  <Card title="Cursor Setup" icon="window" href="/integrations/cursor">
