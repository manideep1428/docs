---
title: Kilocode Integration
---

> Configure Kilocode VSCode extension to use emby

Kilocode is a powerful VSCode extension that provides AI-powered code completion, inline chat, and code actions. Configure it to use emby for access to multiple AI models.
...
<Tabs>
  ...
  <Tab title="Via settings.json">
    ...
    **Location**: `.vscode/settings.json` or `~/.config/Code/User/settings.json`
    ...
    <Tip>
      You can use environment variables: `"apiKey": "${env:EMBY_API_KEY}"`
    </Tip>
  </Tab>
...
<Steps>
  ...
  <Step title="Get emby API Key">
    1. Visit [emby Dashboard](https://emby.io/dashboard)
  ...
  <Step title="Configure Extension">
    ...
    ```json  theme={null}
    {
      ...
      "kilocode.customProvider": {
        "name": "emby",
        "baseURL": "https://dev.emby.ai/v1",
        "apiKey": "sk-emby-your-actual-key-here"
      },
      "kilocode.defaultModel": "gpt-5"
    }
    ```
...
2. emby API key ([Get one](https://emby.io/dashboard))

### Configuration

1. **Set environment variable:**
   ```bash
   export EMBY_API_KEY="your-key-here"
````
...
export EMBY_API_KEY="their-personal-key"
...
Switching from Copilot to Kilocode with emby:
    ...
    **Solutions:**
    ...
    2. **Verify configuration:**
       ```json  theme={null}
       {
         ...
         "kilocode.customProvider": {
           "baseURL": "https://dev.emby.ai/v1"  // No trailing slash
         }
       }
       ```
  ...
  <Accordion title="High latency / slow completions">
    ...
    **Solutions:**
    ...
    4. **Check network:**
       ```bash  theme={null}
       ping ai.emby.io
       curl -w "@-" -o /dev/null -s https://dev.emby.ai/v1/models <<'EOF'
  ...
  <Accordion title="API key not recognized">
    ...
    **Solutions:**
    ...
    2. **Check key is active:**
       * Login to [Dashboard](https://emby.io/dashboard)
       * Go to API Keys
       * Verify key is not revoked/expired

    3. **Test key directly:**
       ```bash  theme={null}
       curl -H "Authorization: Bearer sk-emby-your-key" \
...
<CardGroup cols={2}>
  <Card title="Use Environment Variables" icon="key">
    Keep API keys in env vars, reference with `${env:EMBY_API_KEY}`
  </Card>
  ...
  <Card title="Monitor Token Usage" icon="chart-line">
    Check [Dashboard](https://emby.io/dashboard) to optimize costs
  </Card>
</CardGroup>
