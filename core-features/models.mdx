...
export const ModelsCatalog = ({baseUrl = 'https://ai.megallm.io', useSampleOnError = true}) => {
  ...
  const filterModelsByProvider = (models, provider) => {
    ...
    return models.filter(model => {
      ...
      switch (provider) {
        case 'OpenAI':
          return family.includes('openai');
        case 'Anthropic':
          return family.includes('anthropic');
        case 'Google':
          return family.includes('google');
        case 'Meta':
          return family.includes('meta');
        case 'Other':
          return !family.includes('openai') && !family.includes('anthropic') && !family.includes('google') && !family.includes('meta');
        default:
          return false;
      }
    });
    ...
    const get = m => {
      switch (sort.key) {
        ...
        case 'inputPrice':
          ...
          return promptPrice ? (typeof promptPrice === 'string' ? parseFloat(promptPrice) : promptPrice) : Infinity;
        case 'outputPrice':
          const completionPrice = m.pricing?.completion;
          return completionPrice ? (typeof completionPrice === 'string' ? parseFloat(completionPrice) : completionPrice) : Infinity;
        default:
          return m.id || '';
      }
    };
  ...
  return <div>
      ...
      <div className="mb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          {error && models.length > 0 && <span className="text-xs text-yellow-600 dark:text-yellow-400 font-medium">
              Using sample data (error: {error.split(' – ')[0]})
            </span>}
        </div>
        <div className="flex items-center gap-4">
    ...
    return <tr key={model.id || idx} className={`group border-b border-gray-100 dark:border-gray-700/60 ${idx % 2 === 0 ? 'bg-white dark:bg-gray-900' : 'bg-gray-50/50 dark:bg-gray-800/40'} hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors`}>
                      ...
                      <td className="px-2 py-2 text-center font-semibold text-green-700 dark:text-green-400">{formatPriceValue(model.pricing?.prompt)}</td>
                      {!isEmbedding && <td className="px-2 py-2 text-center font-semibold text-orange-700 dark:text-orange-400">{formatPriceValue(model.pricing?.completion)}</td>}
                    </tr>;
...
<Note>
  ...
  [dashboard](https://dev.emby.ai/models) for real-time pricing and
  availability.
</Note>
...
<CardGroup cols={2}>
  <Card title="Fast Responses" icon="bolt-lightning">
    gpt-5-mini, gpt-4o-mini, gemini-2.0-flash-001, gpt-3.5-turbo
  </Card>

  <Card title="Complex Reasoning" icon="brain-circuit">
  ...
  <Card title="Large Context" icon="file-contract">
    gpt-4.1 (1M+), gemini-2.5-pro (1M+)
    </Card>

  <Card title="Vision Tasks" icon="images">
    gpt-5, gpt-4o, claude-sonnet-4, gemini models
  </Card>

  <Card title="Code Generation" icon="brackets-curly">
...
<CardGroup cols={2}>
  <Card title="Economy" icon="wallet">
...
<Warning>
  **Important**: Model IDs are case-sensitive. Always use the exact model ID as
